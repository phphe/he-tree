  ...
  export default defineComponent({
      ...
      this.$watch(
        ...
        (localVisible: boolean, old?: boolean) => {
          ...
          if (localVisible) {
            ...
            hp.on(document, 'click', this._onDocCick, true)
            //
            this._onUserInput = (e) => {
            ...
            this._offUserInput = hp.extendedListen(
              userInputEventNames.map((eventName) => [
                window,
                eventName,
                this._onUserInput,
              ])
            ...
            this._offGlobalListner = () => {
              hp.off(document, 'click', this._onDocCick, true)
              this._offUserInput()
    ...
    methods: {
      ...
      onParentsScroll() {
        ...
        parents.forEach((el) => hp.on(el, 'scroll', handler))
        this._offParentsScroll = () => {
          parents.forEach((el) => hp.off(el, 'scroll', handler))
        }
      },
      offParentsScroll() {
