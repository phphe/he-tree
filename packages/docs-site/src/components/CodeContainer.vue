<template>
  <div
    class="relative code-container code-container-hightlight"
    :style="sizeStyle"
  >
    <div
      v-show="tab === 'demo' && demo"
      class="not-prose border-2 border-black rounded-md p-2"
    >
      <slot />
    </div>
    <div class="relative" v-show="tab === 'code' || !demo">
      <pre
        ref="pre"
      ><slot name="code-outer"><code :class="'language-'+codeLanguage"><slot name="code">{{ code }}</slot></code></slot></pre>
    ...
    <div class="absolute right-2 -top-8">
      <Transition v-if="!demo || tab === 'code'" name="fade" mode="out-in">
        <VIconMDI
          ...
          :icon="mdiContentCopy"
          :size="20"
          class="cursor-pointer hover:text-primary-400 ml-1"
          :title="$t('message.Copy')"
          v-tooltip="$t('message.Copy')"
      ...
      <VIconMDI
        ...
        :size="22"
        class="cursor-pointer hover:text-primary-400 ml-1"
        :title="$t('message.View Source')"
        v-tooltip="$t('message.View Source')"
        @click.native="tab = 'code'"
      />
    </div>
  </div>
</template>
