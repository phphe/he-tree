...
export default class Draggable extends BaseTree {
  @Prop({ default: "tree-node", type: String }) readonly triggerClass!: string;
  @Prop({ type: Boolean }) readonly triggerBySelf!: boolean;
  @Prop({ type: Boolean, default: true }) readonly draggable!: boolean;
  @Prop({ type: Boolean, default: true }) readonly droppable!: boolean;
  @Prop({ type: [Boolean, Function], default: undefined })
  ...
  @Prop({ type: Number, default: 100 }) readonly placeholderMaxHeight!: number;
  @Prop({ type: Boolean, default: true }) readonly unfoldWhenDragover!: boolean;
  @Prop({ type: Number, default: 80 })
  readonly unfoldWhenDragoverDelay!: number;
  @Prop({ type: Function }) readonly isNodeUnfoldable!: (
    store: Store3
  ) => boolean;
  @Prop({ type: String, default: "top_left_corner" })
  readonly draggingNodePositionMode!: "top_left_corner" | "mouse";

  @Prop({ type: Boolean, default: true })
  readonly preventTextSelection!: boolean;
  // edge scroll
  @Prop({ type: Boolean }) readonly edgeScroll!: boolean;
  @Prop({ type: Number, default: 50 })
  readonly edgeScrollTriggerMargin!: number;
  @Prop({ type: Number, default: 0.35 }) readonly edgeScrollSpeed!: number;
  @Prop({ type: String, default: "top_left_corner" })
  readonly edgeScrollTriggerMode!: "top_left_corner" | "mouse";
  @Prop() readonly edgeScrollSpecifiedContainerX!:
    | HTMLElement
    | ((store: Store3) => HTMLElement);
  @Prop() readonly edgeScrollSpecifiedContainerY!:
    | HTMLElement
    | ((store: Store3) => HTMLElement);
